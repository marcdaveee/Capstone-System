<?php 
  ...
  if($_SERVER["REQUEST_METHOD"] == "GET"){
    ...
    if($selectedRecord){
        $productId = $selectedRecord["product_id"];
        $softwareName = $selectedRecord["software"];
        $softwareType = $selectedRecord["software_type"];
        $manufacturer = $selectedRecord["manufacturer"];
        $dateOfPurchase = $selectedRecord["date_of_purchase"];
        $numOfInstallation = $selectedRecord["no_of_installation"];
        $validity = $selectedRecord["validity"];
        $softwDescription = $selectedRecord["soft_description"];                
        $currStatus = $selectedRecord["curr_status"];        
    }
    else{
  ...
  if(isset($_POST["submit"])){    
    ...
    if(empty($_POST["soft-product-id"])){
      $error["productIdError"] = "*required";
    }
    else{
      $productId = htmlspecialchars($_POST["soft-product-id"]);
    ...
    if($error["productIdError"] == "" && $error["softwareNameError"] == "" && $error["softwareTypeError"] == "" && $error["manufacturerFieldError"] == "" && $error["dateOfPurchaseError"] == "" && $error["noOfInstallationError"] == "" && $error["softwDescriptionError"] == ""){
        ...
        SET product_id='$productId',
            ...
            no_of_installation='$numOfInstallation',
            validity='$validity',
            soft_description='$softwDescription',
  ...
  function ifExist($value){
    ...
    foreach($assets as $asset){
      if(strtoupper($asset["product_id"]) == strtoupper($value) && $asset["id"] != $_GET["current_id"]){                
        return true;
      }
    }
    ...
    <section class="main-dashboard">
      ...
      <section class="add-new-asset">
        ...
        <div class="top-header">
          <a href="<?php echo "software_asset_info.php?id=".$_GET["current_id"]?>" class="back btn btn-outline"><i class="fas fa-chevron-left"></i> Back</a>
          <h1 class="txt-lg">Update Software Asset</h1>
          ...
          <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" class="add-software-asset-form">
            ...
            <div class="add-asset-form-container">
               ...
               <div class="form-group">
                <label for="soft-product-id">Product ID</label>
                <input type="text" id="soft-product-id" name="soft-product-id" value="<?php echo $productId ?>" placeholder="Enter the Product ID of the software">
