<?php 
  ...
  if(isset($_POST["update"])){
    ...
    if(empty($_POST["current-password"])){
      ...
      if(!isPasswordCorrect($currentPassword, $userEmail)){
  ...
  if(empty($_POST["confirm-password"])){
      ...
      if($password !== htmlspecialchars($_POST["confirm-password"])){
  ...
  function isPasswordCorrect($passwordInput, $userEmail){
    include("../config/db_config.php");

    $sql = "SELECT * FROM user_account_table WHERE email='$userEmail' AND password='".md5($passwordInput)."'";
    $result = mysqli_query($conn, $sql);
  ...
  function ifExist($value){
    ...
    foreach($assets as $asset){
      if(strtoupper($asset["username"]) == strtoupper($value)){                
        return true;
      }
    }
    ...
    <section class="main-dashboard bg-light-primary">
        <div class="container">
          ...
          <div class="grid grid-2 grid-gap-3 px-4 mt-2">
            ...
            <div>
              <h3 class="txt-sm txt-fw-3">Email</h3>
              <div class="flex flex-just-sb form-card">
                <h3 class="txt-sm txt-fw-3 ml-1"><?php echo $userEmail ?></h3>                
              </div>
            </div>
